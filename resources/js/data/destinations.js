// ============================
// FEATURED DESTINATIONS DATA
// ============================

export const destinations = {
    Prague: {
        city: "Prague",
        country: "Czech Republic",
        timezone: "Europe/Prague",
        countryCode: "CZ",
        lat: 50.0755,
        lon: 14.4378,

        countryInfo: {
            currency: "Czech koruna",
            language: "Czech",
            population: 10882341,
            capital: "Prague"
        },

        bestTime: "April to October",
    
        packing: [
            "Comfortable walking shoes",
            "Light jacket",
            "Umbrella"
        ],

        nearbyAttractions: [
            {
                name: "Prague Castle",
                lat: 50.0909,
                lon: 14.4005
            },
            {
                name: "Charles Bridge",
                lat: 50.0865,
                lon: 14.4114
            },
            {
                name: "Old Town Square",
                lat: 50.0870,
                lon: 14.4208
            },
            {
                name: "St. Vitus Cathedral",
                lat: 50.0909,
                lon: 14.4005
            },
            {
                name: "Petřín Lookout Tower",
                lat: 50.0835,
                lon: 14.3955
            }
        ],

        modal: {
            image: "./resources/images/Prague.jpg",

            description:
                "Prague is the enchanting capital of the Czech Republic, famous " +
                "for its medieval architecture, historic landmarks, and picturesque " +
                "views along the Vltava River. With its rich history, vibrant " +
                "culture, and charming atmosphere, it is one of Europe's most " +
                "captivating destinations.",
            
            experiences: [
                "Walk across Charles Bridge",
                "Visit Prague Castle",
                "Explore Old Town Square",
                "Take a boat cruise on the Vltava River",
                "Enjoy panoramic views from Petřín Hill",
                "Discover the historic Jewish Quarter"
            ],
            
            food: [
                "Svíčková",
                "Goulash",
                "Vepřo Knedlo Zelo",
                "Smažený Sýr",
                "Trdelník",
                "Czech Beer"
            ],
            
            hiddenGems: [
                "Vyšehrad",
                "Letná Park",
                "Nový Svět",
                "Vrtba Garden",
                "Divoká Šárka",
                "Náplavka Riverside"
            ],
            
            tips: [
                "Use public transport instead of taxis",
                "Visit popular spots early in the morning",
                "Validate your ticket before boarding trams or the metro",
                "Wear comfortable shoes for Prague's cobblestone streets",
                "Carry some cash for small cafés and markets",
                "Enjoy tap water—it's safe to drink throughout Prague"
            ],
            
            tourismUrl: "https://www.prague.eu"
        }
    },

    Tokyo: {
        city: "Tokyo",
        country: "Japan",
        timezone: "Asia/Tokyo",
        countryCode: "JP",
        lat: 35.6762,
        lon: 139.6503,

        countryInfo: {
            currency: "Japanese yen",
            language: "Japanese",
            population: 123294513,
            capital: "Tokyo"
        },

        bestTime: "March to May and October to November",
        
        packing: [
            "Comfortable walking shoes",
            "Portable umbrella",
            "Cash for small shops"
        ],

        nearbyAttractions: [
            {
                name: "Senso-ji Temple",
                lat: 35.7148,
                lon: 139.7967
            },
            {
                name: "Tokyo Skytree",
                lat: 35.7100,
                lon: 139.8107
            },
            {
                name: "Meiji Jingu",
                lat: 35.6764,
                lon: 139.6993
            },
            {
                name: "Shibuya Scramble Crossing",
                lat: 35.6595,
                lon: 139.7005
            },
            {
                name: "Ueno Park",
                lat: 35.7156,
                lon: 139.7745
            }
        ],

        modal: {
            image: "./resources/images/Tokyo.jpg",

            description:
                "Tokyo is Japan's dynamic capital, seamlessly combining ancient " +
                "traditions with modern innovation. Visitors can explore historic " +
                "temples, experience the energy of neon-lit districts, savor " +
                "world-famous cuisine, and immerse themselves in a culture that " +
                "honors its past while embracing the future. Whether you're seeking " +
                "cultural discoveries, culinary adventures, or unforgettable " +
                "cityscapes, Tokyo offers something extraordinary at every turn.",
            
            experiences: [
                "Visit Senso-ji Temple",
                "Cross Shibuya Crossing",
                "Explore TeamLab Planets",
                "Admire the city from Tokyo Skytree",
                "Visit Meiji Shrine",
                "Discover Akihabara's anime and gaming culture"
            ],
            
            food: [
                "Sushi",
                "Ramen",
                "Tempura",
                "Yakitori",
                "Tonkatsu",
                "Matcha Desserts"
            ],
            
            hiddenGems: [
                "Yanaka District",
                "Gotokuji Temple",
                "Todoroki Valley",
                "Kagurazaka",
                "Nezu Shrine",
                "Nakameguro"
            ],
            
            tips: [
                "Purchase a Suica or Pasmo card for transport",
                "Avoid rush hours on major train lines",
                "Carry cash as some smaller shops do not accept cards",
                "Download offline maps and a translation app",
                "Dispose of trash properly, as public bins can be hard to find",
                "Book popular attractions and restaurants in advance"
            ],

            tourismUrl: "https://www.gotokyo.org/en/"
        }
    },

    Paris: {
        city: "Paris",
        country: "France",
        timezone: "Europe/Paris",
        countryCode: "FR",
        lat: 48.8566,
        lon: 2.3522,

        countryInfo: {
            currency: "Euro",
            language: "French",
            population: 68170228,
            capital: "Paris"
        },

        bestTime: "April to June and September to October",
        
        packing: [
            "Comfortable walking shoes",
            "Light jacket",
            "Compact umbrella"
        ],

        nearbyAttractions: [
            {
                name: "Eiffel Tower",
                lat: 48.8584,
                lon: 2.2945
            },
            {
                name: "Louvre Museum",
                lat: 48.8606,
                lon: 2.3376
            },
            {
                name: "Notre-Dame Cathedral",
                lat: 48.8530,
                lon: 2.3499
            },
            {
                name: "Arc de Triomphe",
                lat: 48.8738,
                lon: 2.2950
            },
            {
                name: "Sacré-Cœur Basilica",
                lat: 48.8867,
                lon: 2.3431
            }
        ],

        modal: {
            image: "./resources/images/Paris.jpg",
            
            description:
                "Paris, the City of Light, enchants visitors with its iconic " +
                "monuments, renowned museums, elegant boulevards, and vibrant " +
                "café culture. Whether you're admiring historic architecture, " +
                "exploring world-famous art collections, indulging in French " +
                "cuisine, or strolling along the Seine River, Paris delivers " +
                "a unique blend of romance, culture, and sophistication that " +
                "makes it one of the world's most beloved destinations.",
                
            experiences: [
                "Visit the Eiffel Tower",
                "Explore the Louvre Museum",
                "Walk along the Seine River",
                "Stroll through Montmartre",
                "Visit the Arc de Triomphe",
                "Take a Seine River cruise at sunset"
            ],
            
            food: [
                "Croissant",
                "French Onion Soup",
                "Macarons",
                "Crêpes",
                "Boeuf Bourguignon",
                "Crème Brûlée"
            ],
            
            hiddenGems: [
                "Rue Crémieux",
                "Canal Saint-Martin",
                "Musée de la Vie Romantique",
                "Promenade Plantée",
                "Parc des Buttes-Chaumont",
                "Passage des Panoramas"
            ],
            
            tips: [
                "Book major attractions in advance",
                "Use the Metro to get around efficiently",
                "Visit museums early to avoid crowds",
                "Carry comfortable walking shoes for exploring the city",
                "Keep an eye on your belongings in busy tourist areas",
                "Learn a few basic French phrases for a smoother experience"
            ],
            
            tourismUrl: "https://parisjetaime.com/eng/"
        }
    },

    Tunis: {
        city: "Tunis",
        country: "Tunisia",
        timezone: "Africa/Tunis",
        countryCode: "TN",
        lat: 36.8065,
        lon: 10.1815,

        countryInfo: {
            currency: "Tunisian dinar",
            language: "Arabic",
            population: 12173716,
            capital: "Tunis"
        },

        bestTime: "March to May and September to November",
        
        packing: [
            "Light breathable clothing",
            "Sunscreen",
            "Cash for markets"
        ],

        nearbyAttractions: [
            {
                name: "Medina of Tunis",
                lat: 36.7992,
                lon: 10.1710
            },
            {
                name: "Bardo National Museum",
                lat: 36.8092,
                lon: 10.1332
            },
            {
                name: "Zitouna Mosque",
                lat: 36.7986,
                lon: 10.1718
            },
            {
                name: "Sidi Bou Said",
                lat: 36.8703,
                lon: 10.3417
            },
            {
                name: "Carthage Archaeological Site",
                lat: 36.8529,
                lon: 10.3230
            }
        ],

        modal: {
            image: "./resources/images/Tunis.jpg",
            
            description:
                "Tunis, the capital of Tunisia, combines Mediterranean beauty, " +
                "rich cultural heritage, and vibrant local life. Wander through " +
                "the UNESCO-listed Medina, explore lively markets filled with " +
                "traditional crafts, savor flavorful Tunisian cuisine, and visit " +
                "remarkable archaeological sites that tell the story of civilizations " +
                "spanning thousands of years. With its warm hospitality and diverse " +
                "attractions, Tunis offers an unforgettable gateway to North African " +
                "culture and history.",

            experiences: [
                "Explore the Medina of Tunis",
                "Visit the ancient ruins of Carthage",
                "Stroll through the picturesque village of Sidi Bou Said",
                "Discover the Bardo National Museum",
                "Walk along Avenue Habib Bourguiba",
                "Visit the Zitouna Mosque"
            ],
            
            food: [
                "Couscous",
                "Brik",
                "Lablabi",
                "Ojja",
                "Makroudh",
                "Bambalouni"
            ],
            
            hiddenGems: [
                "Dar Lasram",
                "Belvedere Park",
                "Marsa Corniche",
                "Ennejma Ezzahra Palace",
                "Tourbet El Bey",
                "The Andalusian Quarter of the Medina"
            ],
            
            tips: [
                "Visit the Medina early to avoid crowds",
                "Carry cash for local markets and small shops",
                "Dress modestly when visiting religious sites",
                "Negotiate politely when shopping in traditional souks",
                "Use official taxis and ensure the meter is running",
                "Stay hydrated during the warmer months"
            ],
            
            tourismUrl: "https://www.discovertunisia.com"
        }
    },

    Kirkjufell: {
        city: "Kirkjufell",
        country: "Iceland",
        timezone: "Atlantic/Reykjavik",
        countryCode: "IS",
        lat: 64.9265,
        lon: -23.3065,

        countryInfo: {
            currency: "Icelandic króna",
            language: "Icelandic",
            population: 393600,
            capital: "Reykjavík"
        },

        bestTime: "June to September for hiking, September to March for Northern Lights",
        
        packing: [
            "Waterproof jacket",
            "Warm layers",
            "Sturdy hiking shoes"
        ],

        nearbyAttractions: [
            {
                name: "Kirkjufellsfoss",
                lat: 64.9419,
                lon: -23.3066
            },
            {
                name: "Grundarfjörður Harbor",
                lat: 64.9244,
                lon: -23.2631
            },
            {
                name: "Saxhóll Crater",
                lat: 64.8660,
                lon: -23.8258
            },
            {
                name: "Djúpalónssandur Beach",
                lat: 64.7515,
                lon: -23.8950
            },
            {
                name: "Lóndrangar Cliffs",
                lat: 64.7394,
                lon: -23.7754
            }
        ],

        modal: {
            image: "./resources/images/Iceland.jpg",
            
            description:
                "Kirkjufell, one of Iceland's most iconic and photographed " +
                "mountains, captivates visitors with its distinctive peak, " +
                "dramatic landscapes, and stunning natural beauty. Surrounded " +
                "by rugged coastlines, cascading waterfalls, and panoramic views " +
                "of the Snæfellsnes Peninsula, this remarkable landmark offers " +
                "unforgettable opportunities for hiking, photography, and " +
                "Northern Lights viewing. Whether bathed in the midnight sun or " +
                "covered in winter snow, Kirkjufell showcases the raw beauty of " +
                "Iceland's wilderness.",
            
            experiences: [
                "Photograph Kirkjufell and Kirkjufellsfoss Waterfall",
                "Explore the Snæfellsnes Peninsula",
                "Watch the Northern Lights during winter",
                "Hike scenic trails around Kirkjufell",
                "Explore Snæfellsjökull National Park",
                "Experience the midnight sun in summer"
            ],
            
            food: [
                "Icelandic Lamb Soup",
                "Fresh Atlantic Cod",
                "Skyr",
                "Arctic Char",
                "Lobster Soup",
                "Rúgbrauð"
            ],
            
            hiddenGems: [
                "Grundarfjörður Harbor",
                "Saxhóll Crater",
                "Djúpalónssandur Black Sand Beach",
                "Rauðfeldsgjá Gorge",
                "Búðir Black Church",
                "Ytri Tunga Seal Beach"
            ],
            
            tips: [
                "Dress in layers as weather changes quickly",
                "Visit at sunrise or sunset for the best photography",
                "Check road and weather conditions before traveling",
                "Wear waterproof clothing and sturdy hiking shoes",
                "Respect marked trails and fragile moss-covered terrain",
                "Bring a tripod if you plan to photograph the Northern Lights"
            ],
            
            tourismUrl: "https://www.visiticeland.com"
        }
    },

    Thailand: {
        city: "Bangkok",
        country: "Thailand",
        timezone: "Asia/Bangkok",
        countryCode: "TH",
        lat: 13.7563,
        lon: 100.5018,

        countryInfo: {
            currency: "Thai baht",
            language: "Thai",
            population: 65818726,
            capital: "Bangkok"
        },

        bestTime: "November to February",
        
        packing: [
            "Light breathable clothing",
            "Mosquito repellent",
            "Sunscreen"
        ],

        nearbyAttractions: [
            {
                name: "Grand Palace",
                lat: 13.7500,
                lon: 100.4913
            },
            {
                name: "Wat Arun",
                lat: 13.7437,
                lon: 100.4889
            },
            {
                name: "Phi Phi Islands",
                lat: 7.7407,
                lon: 98.7784
            },
            {
                name: "Khao Sok National Park",
                lat: 8.9298,
                lon: 98.5311
            },
            {
                name: "Floating Market",
                lat: 13.5183,
                lon: 99.9588
            }
        ],

        modal: {
            image: "./resources/images/thailand.jpg",
            
            description:
                "Thailand is a vibrant Southeast Asian destination renowned for " +
                "its golden temples, tropical islands, bustling markets, and rich " +
                "cultural heritage. From the energetic streets of Bangkok and the " +
                "ancient temples of Chiang Mai to the crystal-clear waters and " +
                "white-sand beaches of its southern coast, Thailand offers an " +
                "extraordinary blend of adventure, relaxation, and authentic local " +
                "experiences. Combined with its warm hospitality and world-famous " +
                "cuisine, the Land of Smiles remains one of the world's most " +
                "captivating travel destinations.",
            
            experiences: [
                "Visit the Grand Palace in Bangkok",
                "Explore the floating markets",
                "Relax on the beaches of Phuket and Krabi",
                "Discover the ancient temples of Chiang Mai",
                "Island-hop through Phi Phi Islands",
                "Experience an ethical elephant sanctuary"
            ],
            
            food: [
                "Pad Thai",
                "Tom Yum Goong",
                "Mango Sticky Rice",
                "Green Curry",
                "Som Tam",
                "Khao Soi"
            ],
            
            hiddenGems: [
                "Koh Lanta",
                "Wat Samphran Dragon Temple",
                "Khao Sok National Park",
                "Koh Yao Noi",
                "Samet Nangshe Viewpoint",
                "Ban Rak Thai"
            ],
            
            tips: [
                "Respect local customs when visiting temples",
                "Stay hydrated in the tropical climate",
                "Use local transportation to experience everyday Thai life",
                "Dress modestly when entering temples and sacred sites",
                "Carry cash for street food vendors and local markets",
                "Keep mosquito repellent handy, especially in rural areas"
            ],
            
            tourismUrl: "https://www.tourismthailand.org"
        }
    },

    Rome: {
        city: "Rome",
        country: "Italy",
        timezone: "Europe/Rome",
        countryCode: "IT",
        lat: 41.9028,
        lon: 12.4964,

        countryInfo: {
            currency: "Euro",
            language: "Italian",
            population: 58870762,
            capital: "Rome"
        },

        bestTime: "April to June and September to October",
        
        packing: [
            "Comfortable walking shoes",
            "Reusable water bottle",
            "Light jacket"
        ],

        nearbyAttractions: [
            {
                name: "Colosseum",
                lat: 41.8902,
                lon: 12.4922
            },
            {
                name: "Trevi Fountain",
                lat: 41.9009,
                lon: 12.4833
            },
            {
                name: "Pantheon",
                lat: 41.8986,
                lon: 12.4769
            },
            {
                name: "Roman Forum",
                lat: 41.8925,
                lon: 12.4853
            },
            {
                name: "St. Peter's Basilica",
                lat: 41.9022,
                lon: 12.4539
            }
        ],

        modal: {
            image: "./resources/images/Rome.jpg",
            
            description:
                "Rome, the Eternal City, is a captivating destination where " +
                "ancient history and modern life intertwine. Home to iconic " +
                "landmarks, magnificent architecture, vibrant piazzas, and " +
                "world-renowned Italian cuisine, the city offers an unforgettable " +
                "journey through centuries of culture and heritage. From exploring " +
                "the mighty Colosseum and wandering through charming cobblestone " +
                "streets to savoring authentic pasta and gelato, Rome enchants " +
                "visitors with its timeless beauty, rich traditions, and lively " +
                "atmosphere.",
            
            experiences: [
                "Explore the Colosseum",
                "Visit the Vatican Museums and St. Peter's Basilica",
                "Throw a coin into the Trevi Fountain",
                "Walk through the Roman Forum",
                "Admire the Pantheon",
                "Discover the charming Trastevere neighborhood"
            ],
            
            food: [
                "Carbonara",
                "Cacio e Pepe",
                "Supplì",
                "Amatriciana",
                "Gelato",
                "Tiramisu"
            ],
            
            hiddenGems: [
                "Quartiere Coppedè",
                "Orange Garden (Giardino degli Aranci)",
                "Basilica di San Clemente",
                "Aventine Keyhole",
                "Via Appia Antica",
                "Gianicolo Hill"
            ],
            
            tips: [
                "Book major attractions in advance",
                "Carry a reusable water bottle and use Rome's public fountains",
                "Visit famous landmarks early in the morning or late in the evening",
                "Wear comfortable shoes for Rome's cobblestone streets",
                "Validate public transport tickets before boarding",
                "Keep an eye on your belongings in crowded tourist areas"
            ],
            
            tourismUrl: "https://www.turismoroma.it/en"
        }
    },

    Santorini: {
        city: "Santorini",
        country: "Greece",
        timezone: "Europe/Athens",
        countryCode: "GR",
        lat: 36.3932,
        lon: 25.4615,

        countryInfo: {
            currency: "Euro",
            language: "Greek",
            population: 10413982,
            capital: "Athens"
        },

        bestTime: "April to June and September to October",
        
        packing: [
            "Sunscreen",
            "Comfortable sandals",
            "Light summer clothing"
        ],

        nearbyAttractions: [
            {
                name: "Oia Village",
                lat: 36.4618,
                lon: 25.3753
            },
            {
                name: "Fira",
                lat: 36.4167,
                lon: 25.4319
            },
            {
                name: "Red Beach",
                lat: 36.3496,
                lon: 25.3964
            },
            {
                name: "Akrotiri Archaeological Site",
                lat: 36.3511,
                lon: 25.4033
            },
            {
                name: "Skaros Rock",
                lat: 36.4316,
                lon: 25.4228
            }
        ],

        modal: {
            image: "./resources/images/santorini.jpg",
            
            description:
                "Santorini is a breathtaking Greek island renowned for its " +
                "whitewashed villages, blue-domed churches, dramatic volcanic " +
                "cliffs, and unforgettable sunsets over the Aegean Sea. Perched " +
                "along the edge of a spectacular caldera, the island offers a " +
                "perfect blend of natural beauty, rich history, and Mediterranean " +
                "charm. From exploring picturesque cliffside towns and relaxing on " +
                "unique volcanic beaches to savoring fresh local cuisine and " +
                "watching the sun dip below the horizon, Santorini promises an " +
                "extraordinary and romantic island escape.",
            
            experiences: [
                "Watch the sunset in Oia",
                "Explore the ancient ruins of Akrotiri",
                "Sail around the volcanic caldera",
                "Walk the scenic trail from Fira to Oia",
                "Relax on Red Beach",
                "Enjoy a traditional Greek wine tasting"
            ],
            
            food: [
                "Tomatokeftedes (Tomato Fritters)",
                "Fresh Grilled Seafood",
                "Fava Santorinis",
                "Santorini White Eggplant",
                "Greek Salad",
                "Local Assyrtiko Wine"
            ],
            
            hiddenGems: [
                "Pyrgos Village",
                "Vlychada Beach",
                "Megalochori",
                "Ammoudi Bay",
                "Profitis Ilias Monastery",
                "Skaros Rock"
            ],
            
            tips: [
                "Visit Oia early in the morning to avoid crowds",
                "Wear comfortable shoes for the island's steep paths",
                "Book accommodations and activities well in advance during summer",
                "Carry water and sun protection, especially during peak season",
                "Reserve sunset-view restaurants ahead of time",
                "Explore lesser-known villages for a more authentic experience"
            ],
            
            tourismUrl: "https://www.santorini.gr"
        }
    },

    NewYorkCity: {
        city: "New York City",
        country: "United States",
        timezone: "America/New_York",
        countryCode: "US",
        lat: 40.7128,
        lon: -74.0060,

        countryInfo: {
            currency: "United States dollar",
            language: "English",
            population: 340110988,
            capital: "Washington, D.C."
        },

        bestTime: "April to June and September to November",
        
        packing: [
            "Comfortable walking shoes",
            "Weather-appropriate layers",
            "Portable phone charger"
        ],

        nearbyAttractions: [
            {
                name: "Statue of Liberty",
                lat: 40.6892,
                lon: -74.0445
            },
            {
                name: "Central Park",
                lat: 40.7829,
                lon: -73.9654
            },
            {
                name: "Times Square",
                lat: 40.7580,
                lon: -73.9855
            },
            {
                name: "Brooklyn Bridge",
                lat: 40.7061,
                lon: -73.9969
            },
            {
                name: "Empire State Building",
                lat: 40.7484,
                lon: -73.9857
            }
        ],
        
        modal: {
            image: "./resources/images/new-york-city.jpg",
            
            description:
                "New York City is one of the world's most iconic destinations, " +
                "famous for its towering skyscrapers, diverse neighborhoods, " +
                "renowned cultural institutions, and vibrant energy. From the bright " +
                "lights of Times Square and the green spaces of Central Park to " +
                "world-class museums and unforgettable skyline views, New York City " +
                "offers an exciting blend of history, culture, entertainment, and " +
                "urban adventure.",
                
            experiences: [
                "Visit Times Square",
                "Walk through Central Park",
                "See the Statue of Liberty",
                "Admire views from the Empire State Building",
                "Explore the Metropolitan Museum of Art",
                "Walk across the Brooklyn Bridge"
            ],
            
            food: [
                "New York Style Pizza",
                "Bagels",
                "Cheesecake",
                "Pastrami Sandwich",
                "Hot Dogs",
                "Pretzels"
            ],
            
            hiddenGems: [
                "Little Island",
                "Roosevelt Island Tramway",
                "The High Line",
                "Green-Wood Cemetery",
                "City Island",
                "The Cloisters"
            ],
            
            tips: [
                "Use the subway for fast and affordable transportation",
                "Purchase attraction tickets online in advance",
                "Wear comfortable shoes for walking",
                "Avoid Times Square restaurants for better local dining",
                "Visit popular attractions early in the day",
                "Keep an eye on your belongings in crowded areas"
            ],
            
            tourismUrl: "https://www.nycgo.com"
        }
    },
    
    CapeTown: {
        city: "Cape Town",
        country: "South Africa",
        timezone: "Africa/Johannesburg",
        countryCode: "ZA",
        lat: -33.9249,
        lon: 18.4241,

        countryInfo: {
            currency: "South African rand",
            language: "English",
            population: 62041410,
            capital: "Pretoria"
        },

        bestTime: "March to May and September to November",
        
        packing: [
            "Sunscreen",
            "Light jacket",
            "Comfortable walking shoes"
        ],

        nearbyAttractions: [
            {
                name: "Table Mountain",
                lat: -33.9628,
                lon: 18.4098
            },
            {
                name: "Cape of Good Hope",
                lat: -34.3568,
                lon: 18.4740
            },
            {
                name: "Boulders Beach",
                lat: -34.1961,
                lon: 18.4516
            },
            {
                name: "V&A Waterfront",
                lat: -33.9036,
                lon: 18.4202
            },
            {
                name: "Kirstenbosch National Botanical Garden",
                lat: -33.9875,
                lon: 18.4327
            }
        ],
        
        modal: {
            image: "./resources/images/cape-town.jpg",

            description:
                "Cape Town is one of Africa's most spectacular destinations, " +
                "surrounded by dramatic mountains, pristine beaches, and rich " +
                "cultural heritage. From the iconic Table Mountain and vibrant " +
                "Waterfront to nearby vineyards and wildlife experiences, Cape Town " +
                "offers an unforgettable mix of nature, adventure, and city life.",
                
            experiences: [
                "Ride the cable car up Table Mountain",
                "Visit the V&A Waterfront",
                "Explore Robben Island",
                "Drive along Chapman's Peak",
                "Visit Boulders Beach penguins",
                "Tour the Cape Winelands"
            ],
            
            food: [
                "Bobotie",
                "Braai",
                "Bunny Chow",
                "Malva Pudding",
                "Cape Malay Curry",
                "Fresh Seafood"
            ],
            
            hiddenGems: [
                "Kalk Bay",
                "Muizenberg",
                "Kirstenbosch Boomslang Walkway",
                "Noordhoek Beach",
                "Woodstock Street Art",
                "Silvermine Nature Reserve"
            ],
            
            tips: [
                "Book Table Mountain visits on clear-weather days",
                "Keep valuables out of sight",
                "Use ride-hailing apps for convenience",
                "Bring sunscreen throughout the year",
                "Explore beyond the city center",
                "Reserve wine tours in advance"
            ],
            
            tourismUrl: "https://www.capetown.travel"
        }
    },
    
    Bali: {
        city: "Bali",
        country: "Indonesia",
        timezone: "Asia/Makassar",
        countryCode: "ID",
        lat: -8.3405,
        lon: 115.0920,

        countryInfo: {
            currency: "Indonesian rupiah",
            language: "Indonesian",
            population: 281190067,
            capital: "Jakarta"
        },

        bestTime: "April to October",
        
        packing: [
            "Light breathable clothing",
            "Mosquito repellent",
            "Temple-appropriate outfit"
        ],

        nearbyAttractions: [
            {
                name: "Uluwatu Temple",
                lat: -8.8291,
                lon: 115.0849
            },
            {
                name: "Tegallalang Rice Terraces",
                lat: -8.4316,
                lon: 115.2793
            },
            {
                name: "Sacred Monkey Forest Sanctuary",
                lat: -8.5195,
                lon: 115.2593
            },
            {
                name: "Tanah Lot Temple",
                lat: -8.6212,
                lon: 115.0868
            },
            {
                name: "Mount Batur",
                lat: -8.2420,
                lon: 115.3753
            }
        ],
        
        modal: {
            image: "./resources/images/bali.jpg",

            description:
                "Bali is a tropical paradise known for its lush rice terraces, " +
                "beautiful beaches, spiritual temples, and vibrant culture. Whether " +
                "you're seeking relaxation, adventure, wellness retreats, or " +
                "stunning landscapes, Bali offers an unforgettable island experience.",
                
            experiences: [
                "Visit the Tegalalang Rice Terraces",
                "Explore Uluwatu Temple",
                "Watch the sunset at Tanah Lot",
                "Relax on Seminyak Beach",
                "Discover Ubud's cultural attractions",
                "Snorkel around Nusa Penida"
            ],
            
            food: [
                "Nasi Goreng",
                "Mie Goreng",
                "Satay",
                "Babi Guling",
                "Gado-Gado",
                "Pisang Goreng"
            ],
            
            hiddenGems: [
                "Sidemen Valley",
                "Tukad Cepung Waterfall",
                "Amed",
                "Jatiluwih Rice Terraces",
                "Banyumala Twin Waterfalls",
                "Pemuteran"
            ],
            
            tips: [
                "Rent a scooter only if you're comfortable driving one",
                "Carry cash for small local businesses",
                "Dress respectfully when visiting temples",
                "Avoid peak traffic hours around popular areas",
                "Stay hydrated in the tropical climate",
                "Book popular beach clubs and tours in advance"
            ],
            
            tourismUrl: "https://www.indonesia.travel"
        }
    },
    
    Banff: {
        city: "Banff National Park",
        country: "Canada",
        timezone: "America/Edmonton",
        countryCode: "CA",
        lat: 51.4968,
        lon: -115.9281,

        countryInfo: {
            currency: "Canadian dollar",
            language: "English",
            population: 39742430,
            capital: "Ottawa"
        },

        bestTime: "June to September for hiking, December to March for skiing",
        
        packing: [
            "Warm layers",
            "Hiking shoes",
            "Reusable water bottle"
        ],

        nearbyAttractions: [
            {
                name: "Lake Louise",
                lat: 51.4254,
                lon: -116.1773
            },
            {
                name: "Moraine Lake",
                lat: 51.3215,
                lon: -116.1860
            },
            {
                name: "Banff Gondola",
                lat: 51.1499,
                lon: -115.5735
            },
            {
                name: "Johnston Canyon",
                lat: 51.2447,
                lon: -115.8398
            },
            {
                name: "Bow Falls",
                lat: 51.1681,
                lon: -115.5605
            }
        ],
        
        modal: {
            image: "./resources/images/banff.jpg",
            
            description:
                "Banff National Park is Canada's oldest national park and one of " +
                "the most breathtaking mountain destinations in the world. Famous " +
                "for turquoise lakes, towering peaks, abundant wildlife, and scenic " +
                "adventures, Banff offers year-round opportunities for exploration " +
                "and outdoor recreation.",
                
            experiences: [
                "Visit Lake Louise",
                "Explore Moraine Lake",
                "Ride the Banff Gondola",
                "Drive the Icefields Parkway",
                "Spot wildlife in the Rockies",
                "Hike Johnston Canyon"
            ],
            
            food: [
                "Alberta Beef",
                "Poutine",
                "Maple Desserts",
                "Bison Burgers",
                "Nanaimo Bars",
                "Local Craft Beer"
            ],
            
            hiddenGems: [
                "Peyto Lake Viewpoint",
                "Sunshine Meadows",
                "Bow Lake",
                "Lake Minnewanka",
                "Cascade Ponds",
                "Vermilion Lakes"
            ],
            
            tips: [
                "Arrive early for popular lakes and viewpoints",
                "Carry layers as mountain weather changes quickly",
                "Respect wildlife and keep a safe distance",
                "Book accommodation well in advance during summer",
                "Bring hiking shoes for trails",
                "Purchase park passes before arrival"
            ],
            
            tourismUrl: "https://www.banfflakelouise.com"
        }
    },

    RioDeJaneiro: {
        city: "Rio de Janeiro",
        country: "Brazil",
        timezone: "America/Sao_Paulo",
        countryCode: "BR",
        lat: -22.9068,
        lon: -43.1729,
        
        countryInfo: {
            currency: "Brazilian real",
            language: "Portuguese",
            population: 203062512,
            capital: "Brasília"
        },

        bestTime: "May to October",
        
        packing: [
            "Sunscreen",
            "Beachwear",
            "Comfortable walking shoes"
        ],

        nearbyAttractions: [
            {
                name: "Christ the Redeemer",
                lat: -22.9519,
                lon: -43.2105
            },
            {
                name: "Sugarloaf Mountain",
                lat: -22.9486,
                lon: -43.1566
            },
            {
                name: "Copacabana Beach",
                lat: -22.9711,
                lon: -43.1822
            },
            {
                name: "Ipanema Beach",
                lat: -22.9839,
                lon: -43.2096
            },
            {
                name: "Selarón Steps",
                lat: -22.9153,
                lon: -43.1796
            }
        ],
        
        modal: {
            image: "./resources/images/rio-de-janeiro.jpg",
            
            description:
                "Rio de Janeiro is a vibrant Brazilian city known for its dramatic mountains, " +
                "golden beaches, lively culture, and iconic landmarks. From Christ the Redeemer " +
                "and Sugarloaf Mountain to Copacabana Beach and colorful neighborhoods, Rio offers " +
                "an unforgettable mix of nature, music, history, and urban energy.",
                
            experiences: [
                "Visit Christ the Redeemer",
                "Ride the cable car to Sugarloaf Mountain",
                "Relax on Copacabana Beach",
                "Explore Santa Teresa",
                "Walk along Ipanema Beach",
                "Experience Rio's samba culture"
            ],
            
            food: [
                "Feijoada",
                "Pão de Queijo",
                "Brigadeiro",
                "Coxinha",
                "Açaí Bowl",
                "Churrasco"
            ],
            
            hiddenGems: [
                "Parque Lage",
                "Ilha Fiscal",
                "Pedra do Sal",
                "Lage Park Trails",
                "Vista Chinesa",
                "Prainha Beach"
            ],
            
            tips: [
                "Use official taxis or ride-hailing apps",
                "Avoid displaying valuables in crowded areas",
                "Visit major viewpoints on clear-weather days",
                "Stay near well-known neighborhoods for easier transport",
                "Carry sunscreen for beach days",
                "Check local guidance before hiking"
            ],
            
            tourismUrl: "https://riotur.rio/en/"
        }
    },
    
    Sydney: {
        city: "Sydney",
        country: "Australia",
        timezone: "Australia/Sydney",
        countryCode: "AU",
        lat: -33.8688,
        lon: 151.2093,
        
        countryInfo: {
            currency: "Australian dollar",
            language: "English",
            population: 26638544,
            capital: "Canberra"
        },

        bestTime: "September to November and March to May",
        
        packing: [
            "Sunscreen",
            "Comfortable walking shoes",
            "Light jacket"
        ],

        nearbyAttractions: [
            {
                name: "Sydney Opera House",
                lat: -33.8568,
                lon: 151.2153
            },
            {
                name: "Sydney Harbour Bridge",
                lat: -33.8523,
                lon: 151.2108
            },
            {
                name: "Bondi Beach",
                lat: -33.8908,
                lon: 151.2743
            },
            {
                name: "Royal Botanic Garden",
                lat: -33.8642,
                lon: 151.2166
            },
            {
                name: "Taronga Zoo",
                lat: -33.8430,
                lon: 151.2410
            }
        ],
        
        modal: {
            image: "./resources/images/sydney.jpg",
            
            description:
                "Sydney is Australia's sparkling harbour city, famous for its iconic Opera House, " +
                "Harbour Bridge, golden beaches, and relaxed coastal lifestyle. With world-class " +
                "dining, scenic ferry rides, cultural landmarks, and nearby nature escapes, Sydney " +
                "offers a perfect balance of urban energy and outdoor adventure.",
                
            experiences: [
                "Visit the Sydney Opera House",
                "Walk across the Harbour Bridge",
                "Relax at Bondi Beach",
                "Take a ferry to Manly",
                "Explore The Rocks district",
                "Visit the Royal Botanic Garden"
            ],
            
            food: [
                "Meat Pie",
                "Barramundi",
                "Lamingtons",
                "Pavlova",
                "Fish and Chips",
                "Avocado Toast"
            ],
            
            hiddenGems: [
                "Wendy's Secret Garden",
                "Milk Beach",
                "Barangaroo Reserve",
                "Paddington Reservoir Gardens",
                "Cockatoo Island",
                "Bronte Coastal Walk"
            ],
            
            tips: [
                "Use an Opal card or contactless payment for transport",
                "Book Opera House tours in advance",
                "Wear sunscreen even on cloudy days",
                "Take ferries for scenic harbour views",
                "Swim only between the beach safety flags",
                "Plan extra time for coastal walks"
            ],
            
            tourismUrl: "https://www.sydney.com"
        }
    },
    
    Marrakech: {
        city: "Marrakech",
        country: "Morocco",
        timezone: "Africa/Casablanca",
        countryCode: "MA",
        lat: 31.6295,
        lon: -7.9811,
        
        countryInfo: {
            currency: "Moroccan dirham",
            language: "Arabic",
            population: 37712944,
            capital: "Rabat"
        },

        bestTime: "March to May and September to November",
        
        packing: [
            "Light modest clothing",
            "Cash for souks",
            "Comfortable walking shoes"
        ],

        nearbyAttractions: [
            {
                name: "Jemaa el-Fnaa",
                lat: 31.6258,
                lon: -7.9891
            },
            {
                name: "Bahia Palace",
                lat: 31.6217,
                lon: -7.9811
            },
            {
                name: "Majorelle Garden",
                lat: 31.6416,
                lon: -8.0026
            },
            {
                name: "Koutoubia Mosque",
                lat: 31.6236,
                lon: -7.9936
            },
            {
                name: "Saadian Tombs",
                lat: 31.6170,
                lon: -7.9870
            }
        ],
        
        modal: {
            image: "./resources/images/marrakech.jpg",
            
            description:
                "Marrakech is a captivating Moroccan city known for its colorful souks, historic " +
                "palaces, fragrant gardens, and lively medina. With its rich architecture, traditional " +
                "crafts, rooftop cafés, and nearby desert and mountain excursions, Marrakech offers " +
                "a memorable blend of culture, history, and sensory discovery.",
                
            experiences: [
                "Explore Jemaa el-Fnaa square",
                "Visit Bahia Palace",
                "Wander through the Medina souks",
                "Relax in Majorelle Garden",
                "Visit Koutoubia Mosque",
                "Take a day trip to the Atlas Mountains"
            ],
            
            food: [
                "Tagine",
                "Couscous",
                "Harira",
                "Pastilla",
                "Mint Tea",
                "Msemen"
            ],
            
            hiddenGems: [
                "Le Jardin Secret",
                "Maison de la Photographie",
                "Mellah Quarter",
                "Ben Youssef Madrasa",
                "Agafay Desert",
                "Dar Si Said Museum"
            ],
            
            tips: [
                "Carry cash for souks and small vendors",
                "Negotiate politely when shopping",
                "Dress modestly in traditional areas",
                "Use offline maps inside the medina",
                "Agree on taxi prices before starting the ride",
                "Visit gardens early for a quieter experience"
            ],
            
            tourismUrl: "https://visitmarrakech.com"
        }
    },
    
    Dubai: {
        city: "Dubai",
        country: "United Arab Emirates",
        timezone: "Asia/Dubai",
        countryCode: "AE",
        lat: 25.2048,
        lon: 55.2708,
        
        countryInfo: {
            currency: "United Arab Emirates dirham",
            language: "Arabic",
            population: 9441129,
            capital: "Abu Dhabi"
        },

        bestTime: "November to March",
        
        packing: [
            "Light breathable clothing",
            "Sunscreen",
            "Modest outfit for cultural sites"
        ],

        nearbyAttractions: [
            {
                name: "Burj Khalifa",
                lat: 25.1972,
                lon: 55.2744
            },
            {
                name: "Dubai Mall",
                lat: 25.1985,
                lon: 55.2796
            },
            {
                name: "Palm Jumeirah",
                lat: 25.1124,
                lon: 55.1390
            },
            {
                name: "Dubai Marina",
                lat: 25.0806,
                lon: 55.1404
            },
            {
                name: "Museum of the Future",
                lat: 25.2204,
                lon: 55.2825
            }
        ],
        
        modal: {
            image: "./resources/images/dubai.jpg",
            
            description:
                "Dubai is a modern global destination known for its futuristic skyline, luxury " +
                "shopping, desert adventures, and ambitious architecture. From the Burj Khalifa " +
                "and Dubai Marina to traditional souks and golden desert dunes, the city combines " +
                "innovation, culture, and entertainment in a unique Arabian setting.",
                
            experiences: [
                "Visit the Burj Khalifa",
                "Explore Dubai Marina",
                "Take a desert safari",
                "Shop at Dubai Mall",
                "Visit the Gold and Spice Souks",
                "Relax at Jumeirah Beach"
            ],
            
            food: [
                "Machboos",
                "Shawarma",
                "Luqaimat",
                "Harees",
                "Arabic Coffee",
                "Grilled Seafood"
            ],

            hiddenGems: [
                "Al Fahidi Historical District",
                "Dubai Creek",
                "Ras Al Khor Wildlife Sanctuary",
                "Alserkal Avenue",
                "Hatta",
                "The Coffee Museum"
            ],
            
            tips: [
                "Dress modestly in traditional and religious areas",
                "Use the metro to avoid traffic on busy routes",
                "Book Burj Khalifa tickets in advance",
                "Visit outdoor attractions early or late in the day",
                "Stay hydrated in hot weather",
                "Respect local customs during Ramadan"
            ],
            
            tourismUrl: "https://www.visitdubai.com"
        }
    }
};

